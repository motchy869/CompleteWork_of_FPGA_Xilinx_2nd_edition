...
localparam
    ...
    ADDR_IER            = 7'h08,
    ADDR_ISR            = 7'h0c,
    ADDR_DST_OUT_DATA_0 = 7'h10,
    ADDR_DST_OUT_DATA_1 = 7'h14,
    ADDR_DST_OUT_CTRL   = 7'h18,
...
//------------------------Local signal-------------------
    ...
    reg  [9:0]                    int_xpos = 'b0;
    reg  [9:0]                    int_ypos = 'b0;
    reg  [9:0]                    int_width = 'b0;
    reg  [9:0]                    int_height = 'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_XPOS_DATA_0)
            int_xpos[9:0] <= (WDATA[31:0] & wmask) | (int_xpos[9:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_YPOS_DATA_0)
            int_ypos[9:0] <= (WDATA[31:0] & wmask) | (int_ypos[9:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WIDTH_DATA_0)
            int_width[9:0] <= (WDATA[31:0] & wmask) | (int_width[9:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_HEIGHT_DATA_0)
            int_height[9:0] <= (WDATA[31:0] & wmask) | (int_height[9:0] & ~wmask);
    end
end
