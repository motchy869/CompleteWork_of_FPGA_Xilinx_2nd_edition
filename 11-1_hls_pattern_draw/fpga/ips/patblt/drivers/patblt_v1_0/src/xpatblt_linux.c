...
typedef struct {
    ...
    XPatblt_uio_map maps[ MAX_UIO_MAPS ];
} XPatblt_uio_info;

/***************** Variable Definitions **************************************/
static XPatblt_uio_info uio_info;
...
static int uio_info_read_name(XPatblt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPatblt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPatblt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPatblt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPatblt_Initialize(XPatblt *InstancePtr, const char* InstanceName) {
	XPatblt_uio_info *InfoPtr = &uio_info;
