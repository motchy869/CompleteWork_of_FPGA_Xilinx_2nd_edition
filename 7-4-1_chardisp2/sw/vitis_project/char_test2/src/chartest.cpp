...
FORCE_INLINE void setVramEntry(const int index, const bool blink, const bool inversion, const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t charCode) {
    VRAM[index*4] = charCode;
    VRAM[index*4+1] = ((g<<4)&0xf0)|(b&0xf);
    VRAM[index*4+2] = (VRAM[index*4+2] & 0b11000000) | (blink<<5) | (inversion<<4) | (r&0xf);
}
...
int main() {
    ...
    for (int i=0; i<4000; ++i) {
    	constexpr bool blink = false;
    	const bool inversion = static_cast<bool>((i>>4)&0b1); /* Invert color every 16 characters. */
        setVramEntry(i, blink, inversion, (i>>8)&0xf, (i>>4)&0xf, i&0xf, i&0xff);
    }
