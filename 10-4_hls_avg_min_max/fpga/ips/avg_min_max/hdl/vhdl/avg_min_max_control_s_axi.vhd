...
entity avg_min_max_control_s_axi is
generic (
...
end entity avg_min_max_control_s_axi;
...
architecture behave of avg_min_max_control_s_axi is
    ...
    constant ADDR_AVG_DATA_0 : INTEGER := 16#10#;
    constant ADDR_AVG_CTRL   : INTEGER := 16#14#;
    constant ADDR_MIN_DATA_0 : INTEGER := 16#40#;
    constant ADDR_MIN_CTRL   : INTEGER := 16#44#;
    constant ADDR_MAX_DATA_0 : INTEGER := 16#50#;
    constant ADDR_MAX_CTRL   : INTEGER := 16#54#;
    ...
    signal int_avg_ap_vld      : STD_LOGIC;
    signal int_avg             : UNSIGNED(15 downto 0) := (others => '0');
    signal int_min_ap_vld      : STD_LOGIC;
    signal int_min             : UNSIGNED(15 downto 0) := (others => '0');
    signal int_max_ap_vld      : STD_LOGIC;
    signal int_max             : UNSIGNED(15 downto 0) := (others => '0');
    -- memory signals
    ...
    component avg_min_max_control_s_axi_ram is
        generic (
    ...
    end component avg_min_max_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_a
int_a : avg_min_max_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_AVG_DATA_0 =>
                        rdata_data <= RESIZE(int_avg(15 downto 0), 32);
                    when ADDR_AVG_CTRL =>
                        rdata_data(0) <= int_avg_ap_vld;
                    when ADDR_MIN_DATA_0 =>
                        rdata_data <= RESIZE(int_min(15 downto 0), 32);
                    when ADDR_MIN_CTRL =>
                        rdata_data(0) <= int_min_ap_vld;
                    when ADDR_MAX_DATA_0 =>
                        rdata_data <= RESIZE(int_max(15 downto 0), 32);
                    when ADDR_MAX_CTRL =>
                        rdata_data(0) <= int_max_ap_vld;
                    when others =>
                        NULL;
                    end case;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (avg_ap_vld = '1') then
                    int_avg <= UNSIGNED(avg);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (avg_ap_vld = '1') then
                    int_avg_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_AVG_CTRL) then
                    int_avg_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (min_ap_vld = '1') then
                    int_min <= UNSIGNED(min);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (min_ap_vld = '1') then
                    int_min_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_MIN_CTRL) then
                    int_min_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (max_ap_vld = '1') then
                    int_max <= UNSIGNED(max);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (max_ap_vld = '1') then
                    int_max_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_MAX_CTRL) then
                    int_max_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- a
    int_a_address0       <= SHIFT_RIGHT(UNSIGNED(a_address0), 1)(2 downto 0);
    int_a_ce0            <= a_ce0;
...
entity avg_min_max_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity avg_min_max_control_s_axi_ram;

architecture behave of avg_min_max_control_s_axi_ram is
